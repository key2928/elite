...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($acao === 'pagamento') {
        ...
        if ($duracao) {
            ...
            $pdo->prepare("INSERT INTO pagamentos (aluna_id, treinador_id, plano_id, valor_pago, data_pagamento, data_vencimento, observacao_aluna) VALUES (?,?,?,?,?,?,?)")
                ->execute([$_POST['aluna_id'], $_SESSION['usuario_id'], $plano_id, $_POST['valor'] ?? 0, date('Y-m-d'), $vencimento, $_POST['obs'] ?? '']);
    ...
    if ($acao === 'nova_aluna') {
        ...
        try {
            $pdo->prepare("INSERT INTO usuarios (nome, email, telefone, senha, tipo) VALUES (?,?,?,?,'aluno')")
                ->execute([$_POST['nome'] ?? '', $_POST['email'] ?? '', $_POST['telefone'] ?? '', $senha_hash]);
